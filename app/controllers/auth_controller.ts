...
export default class AuthController {
  ...
  async callback({ params, ally, auth, response }: HttpContext) {
    ...
    let dbUser = await db.selectFrom("users").select("id").where("email", "=", driverUser.email).executeTakeFirst();
    if (!dbUser) {
      let values = { email: driverUser.email, created_at: new Date().getTime(), updated_at: new Date().getTime() }
    ...
    dbUser = await db.selectFrom("users").selectAll().where("email", "=", driverUser.email).executeTakeFirst();
